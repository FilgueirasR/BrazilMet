...
download_AWS_INMET_daily <-function (station, start_date, end_date) {
  ...
  utils::download.file(url = paste0("https://portal.inmet.gov.br/uploads/dadoshistoricos/",
                                    year, ".zip"), destfile = tf, method = "auto", cacheOK = F)
  a <- unzip(zipfile = tf, exdir = outdir, junkpaths = T)
  pasta <- paste0(outdir)
  if (length(list.files(pasta, pattern = station, full.names = T,
                        all.files = T)) == 0) {
    message("There is no data for this period for this station. Choose another period!")
  }
  else {
    list.files(pasta, pattern = station)
    b <- read.csv(file = list.files(pasta, pattern = station,
                                    full.names = T), header = T, sep = ";", skip = 8,
    ...
    dfx$Hora <- format(as.POSIXct(dfx$date_hora, format = "%Y-%m-%d %H"),
                       "%H")
    if (nrow(dfx) < 4380) {
    }
    else {
      dfx_temp <- na.omit(dplyr::select(dfx, Hora, Data,
                                        `to-min(C)`, `to-max(C)`, `tar_min(C)`, `tar_max(C)`,
                                        `tbs(C)`))
      n_dfx_temp <- group_by(dfx_temp, Data) %>% summarise(n = n()) %>%
        filter(n == 24)
      if (nrow(n_dfx_temp) == 0) {
        ...
        dfx_temp <- dplyr::mutate(dfx_temp, to_mean = (`to-min(C)` +
                                                         `to-max(C)`)/2)
        dfx_temp_mean_day <- aggregate(tar_mean ~ Data,
                                       dfx_temp, mean)
        dfx_temp_min_day <- aggregate(`tar_min(C)` ~
                                        Data, dfx_temp, min)
        dfx_temp_max_day <- aggregate(`tar_max(C)` ~
                                        Data, dfx_temp, max)
        dfx_to_min_day <- aggregate(`to-min(C)` ~ Data,
                                    dfx_temp, min)
        dfx_to_max_day <- aggregate(`to-max(C)` ~ Data,
                                    dfx_temp, max)
        dfx_to_mean_day <- aggregate(to_mean ~ Data,
                                     dfx_temp, mean)
        dfx_tbs_day <- aggregate(`tbs(C)` ~ Data, dfx_temp,
                                 mean)
        dfx_temps_day <- cbind(dfx_temp_mean_day, dfx_temp_min_day,
                               dfx_temp_max_day, dfx_to_mean_day, dfx_to_min_day,
                               dfx_to_max_day, dfx_tbs_day)
        dfx_temps_day <- dplyr::select(dfx_temps_day,
                                       -3, -5, -7, -9, -11, -13)
        dfx_prec <- na.omit(dplyr::select(dfx, Hora,
                                          Data, `ppt-h (mm)`))
        dfx_prec <- group_by(dfx_prec, Data)
        if (nrow(dfx_prec) == 0) {
        }
        else {
          dfx_prec_day <- aggregate(`ppt-h (mm)` ~ Data,
                                    dfx_prec, sum)
          dfx_press <- na.omit(dplyr::select(dfx, Hora,
                                             Data, `pressao (mB)`))
          n_dfx_press <- group_by(dfx_press, Data) %>%
            summarise(n = n()) %>% filter(n == 24)
          if (nrow(n_dfx_press) == 0) {
            ...
            dfx_press_mean_day <- aggregate(`pressao (mB)` ~
                                              Data, dfx_press, mean)
            dfx_ur <- na.omit(dplyr::select(dfx, Hora,
                                            Data, `UR-max`, `UR-min`, UR))
            n_dfx_ur <- group_by(dfx_ur, Data) %>% summarise(n = n()) %>%
              filter(n == 24)
            if (nrow(n_dfx_ur) == 0) {
            }
            else {
              ...
              dfx_ur_mean_day <- aggregate(UR ~ Data,
                                           dfx_ur, mean)
              dfx_ur_min_day <- aggregate(`UR-min` ~
                                            Data, dfx_ur, min)
              dfx_ur_max_day <- aggregate(`UR-max` ~
                                            Data, dfx_ur, max)
              dfx_urs_day <- cbind(dfx_ur_mean_day, dfx_ur_max_day,
                                   dfx_ur_min_day)
              dfx_urs_day <- dplyr::select(dfx_urs_day,
                                           -3, -5)
              dfx_vv <- na.omit(dplyr::select(dfx, Hora,
              ...
              if (nrow(n_dfx_vv) == 0) {
                ...
                dfx_vv_mean_day <- aggregate(`U10 (m/s)` ~
                                               Data, dfx_vv, mean)
                dfx_vv_meanu2_day <- aggregate(u2 ~ Data,
                                               dfx_vv, mean)
                dfx_vv_raj_day <- aggregate(`U-raj (m/s)` ~
                                              Data, dfx_vv, max)
                dfx_vv_dir_day <- aggregate(`U-dir(degrees)` ~
                                              Data, dfx_vv, mean)
                dfx_vvs_day <- cbind(dfx_vv_mean_day,
                                     dfx_vv_meanu2_day, dfx_vv_raj_day,
                                     dfx_vv_dir_day)
                dfx_vvs_day <- dplyr::select(dfx_vvs_day,
                                             -3, -5, -7)
                dfx_RG <- dplyr::select(dfx, Hora, Data,
                                        `RG(Kj/m2)`)
                dfx_RG <- dplyr::mutate(dfx_RG, `RG(Mj/m2)` = `RG(Kj/m2)`/1000)
                dfx_RG <- na.omit(dplyr::select(dfx_RG,
                                                -`RG(Kj/m2)`))
                dfx_RG <- dplyr::filter(dfx_RG, `RG(Mj/m2)` >
                                          0)
                n_RG <- group_by(dfx_RG, Data) %>% summarise(n = n()) %>%
                  filter(n >= 12)
                if (nrow(n_RG) == 0) {
                }
                else {
                  ...
                  dfx_RG_sum_day <- aggregate(`RG(Mj/m2)` ~
                                                Data, dfx_RG, sum)
                  julian_day <- as.data.frame(as.numeric(format(dfx_RG_sum_day$Data,
                                                                "%j")))
                  names(julian_day) <- "julian_day"
                  dfx_RG_sum_day <- cbind(dfx_RG_sum_day,
                                          julian_day)
    ...
    df <- df %>% mutate(longitude = longitude, latitude = latitude,
                        altitude = altitude)
    colnames(df) <- c("Date", "Tair_mean (c)", "Tair_min (c)",
                      ...
                      "Station_code", "Longitude (degrees)", "Latitude (degrees)",
                      "Altitude (m)")
    return(df)
  }
}
