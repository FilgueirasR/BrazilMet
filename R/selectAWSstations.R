...
#' @param as_sf logical. Inmet stations inventory result should be export as sf object? Defalut = FALSE.
#'
...
selectAWSstations = function(aoi, as_sf = FALSE){
  ...
  stopifnot(`\`as_sf\` must be logical vector` = is.logical(as_sf) & length(as_sf) == 1,
            `\`aoi\` must be a polygon of class \`sf \` (sf package)` = class(aoi)[1] ==
              "sf")
  
  est = sf::st_as_sf(see_stations_info(), coords = c("Longitude","Latitude")) %>% 
    sf::st_set_crs(4674) %>% 
