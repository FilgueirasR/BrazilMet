...
download_AWS_INMET_daily <- function(station, start_date, end_date) {
  ...
  pasta <- paste0(outdir)
  if (length(list.files(pasta, pattern = station, full.names = T, all.files = T)) == 0) {
    ...
    colnames(df) <- c(
      "Date", "Hour", "ppt-h (mm)", "pressao (mB)", "RG(Mj/m2)", "tbs-(C)", "tpo(C)", "tar_max(C)", "tar_min(C)",
    ...
    names(dfx)
    names(dfx) <- c(
      "Data", "Hora", "ppt-h (mm)", "pressao (mB)", "pressao-max(mB)", "pressao-min(mB)", "RG(Kj/m2)", "tbs(C)", "tpo(C)",
    ...
    for (i in 1:nrow(dfx)) {
      if (longitude > -37.5) (dfx$date_hora[i] <- dfx$date_hora[i] - as.difftime(2, units = "hours")) else if (longitude > -52.5) (dfx$date_hora[i] <- dfx$date_hora[i] - as.difftime(3, units = "hours")) else if (longitude > -67.5) (dfx$date_hora[i] <- dfx$date_hora[i] - as.difftime(4, units = "hours")) else if (longitude > -82.5) (dfx$date_hora[i] <- dfx$date_hora[i] - as.difftime(5, units = "hours"))
    }
    ...
    df <- df %>% mutate(Station = Station, UF = UF, longitude = longitude, latitude = latitude, altitude = altitude)
    colnames(df) <- c(
      "Date", "Tair_mean (c)", "Tair_min (c)", "Tair_max (c)", "Dew_tmean (c)", "Dew_tmin (c)",
      ...
      "DOY", "Ra (Mj m-2 day-1)", "Station_code", "Station", "UF", "Longitude (degrees)", "Latitude (degrees)", "Altitude (m)"
    )
